package com.cloud.whp.auth.endpoint;
...
public class WhpTokenEndpoint {
	private static final String WHP_OAUTH_ACCESS = SecurityConstants.WHP_PREFIX + SecurityConstants.OAUTH_PREFIX + "access:";
	...
	public R<Boolean> delToken(@PathVariable("token") String token, @RequestHeader(required = false) String from) {
		...
		return new R<>(redisTemplate.delete(WHP_OAUTH_ACCESS + token));
	}
	...
	public R<Page> tokenList(@RequestBody Map<String, Object> params, @RequestHeader(required = false) String from) {
		...
		//根据分页参数获取对应数据
		List<String> pages = findKeysForPage(WHP_OAUTH_ACCESS + "*", MapUtil.getInt(params, PaginationConstant.CURRENT), MapUtil.getInt(params, PaginationConstant.SIZE));

		for (String page : pages) {
			String accessToken = StrUtil.subAfter(page, WHP_OAUTH_ACCESS, true);
		...
		result.setTotal(Long.valueOf(redisTemplate.keys(WHP_OAUTH_ACCESS + "*").size()));
		return new R<>(result);
	}
